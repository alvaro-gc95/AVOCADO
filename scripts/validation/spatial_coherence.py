"""
This script check if there are suspicious differences between the data os two correlated stations close to each other
Inputs:
- station to validate
- reference station to compare
...
# Function to check the spatial coherence
def spatial_coherence_test(observations_data, reference_data, climatology_data, err_tolerance, period_to_test):
    ...
    reference = reference_data.reindex(period_to_test).drop(['impossible values',
                                                             'climatological coherence',
                                                             'time step',
                                                             'spatial coherence'], axis=1)
    # Calculate a synthetic time series based on the reference data and the linear regression between both stations
    synthetic_observation = pd.DataFrame(index=period_to_test, columns=['reconstruction'])
    for month, month_dataset in reference.groupby(reference.index.month):
        for hour, hourly_dataset in month_dataset.groupby(month_dataset.index.hour):
            coef = climatology_data.loc[str(hour) + '_' + str(month), reference_station + '_coef']
            intercept = climatology_data.loc[str(hour) + '_' + str(month), reference_station + '_intercept']
            synthetic_observation.loc[hourly_dataset.index] = hourly_dataset.apply(lambda x: x * coef + intercept)

    # Compare the observations with the synthetic series, finding the dates of the values below or above the selected
    # tolerance
    observations_to_test = observations_to_test.join(synthetic_observation)
    print(observations_to_test)
    anomalous_dates = observations_to_test.loc[(observations_to_test['value'] <
                                                observations_to_test['reconstruction'].apply(
                                                    lambda x: x - x * err_tolerance/100)) |
                                               (observations_to_test['value'] >
                                                observations_to_test['reconstruction'].apply(
                                                    lambda x: x + x * err_tolerance/100))].index
...
reference_observations_path = '../data/' + reference_station + '/'
# Tolerance of error between the series made from the regression with the reference station and the real observations
tolerance = 15
...
reference_observations, reference_filename = open_observations(reference_observations_path, observed_variable)
# Open the climatology
climatology = open_climatology(observations_path + 'climatology/', observed_variable)
# Test the spatial coherence coherence in the selected timeframe
spatial_coherence_anomalies = spatial_coherence_test(observations, reference_observations,
                                                     climatology, tolerance, timeframe)
# Label the dates obtained with a "1" if they are suspicious of spatial incoherence, and with a "0" if not
observations.loc[spatial_coherence_anomalies, 'spatial coherence'] = 1
observations.loc[timeframe.drop(spatial_coherence_anomalies), 'spatial coherence'] = 0
